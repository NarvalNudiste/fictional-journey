                ...
                if (isHoldingSomething) {
                    ...
                    if (coll != null) {
                        for (int i = 0; i < coll.Length; i++) {
                            ...
                            else if (coll[i].GetComponent<Transform>().tag == "goal") {
                            coll[i].GetComponent<Goal>().checkFood(heldObject.GetComponent<Food>());
                            heldObject.GetComponent<Rigidbody>().detectCollisions = true;
                            heldObject = null;
                            isHoldingSomething = false;
                            checkNeeded = false;
                    ...
                    if (coll != null) {
                        for (int i = 0; i < coll.Length; i++) {
                            ...
                            else if (coll[i].GetComponent<Transform>().tag == "dispensor") {
                            Transform tempT = coll[i].transform;
                            Transform temp = GameObject.Instantiate(coll[i].GetComponent<Dispensor>().toDispense, new Vector3(tempT.position.x, tempT.position.y + 0.5f, tempT.position.z), Quaternion.identity);
                            heldObject = temp;
